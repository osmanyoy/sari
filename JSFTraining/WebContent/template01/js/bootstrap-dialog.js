...
(function($) {
	...
	BootstrapDialog.prototype = {
		...
		/**
		 * If there is id provided for a button option, it will be in
		 * dialog.indexedButtons list.
		 *
		...
		/**
		 * Set handler for modal event 'show'. This is a setter!
		 *
		 * @param {type}
		 *            onopen
		...
		/**
		 * Set handler for modal event 'hide'. This is a setter!
		 *
		 * @param {type}
		 *            onclose
		 * @returns {_L9.BootstrapDialog.prototype}
		...
		close : function() {
			try {
				var $modalL = this.getModal().modal('hide');
				var $modalNa = $modalL.data('modal');
				var $bodyL = $('body');
				var $modalmanagerL = $bodyL.data('modalmanager');
				$modalmanagerL.destroyModal($modalNa);
			} catch (e) {
				console.log(e);
			}
